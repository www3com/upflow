 ...
 * @returns 包含节点信息的变量列表
 */
export const getAvailableVariablesWithNode = (currentNodeId: string, nodes: Node[], edges: Edge[]) => {
    const variablesWithNode: { nodeId: string; nodeName: string; varName: string; varType: string; nodeIcon: string }[] = [];
    ...
    previousNodes.forEach(node => {
        ...
        // 从开始节点获取输入变量
        if (node.type === NODE_TYPE.START && node.data.input) {
        ...
        variables.forEach(variable => {
            const nodeIcon = NodeTypes[node.type!]?.icon || 'icon-default';
