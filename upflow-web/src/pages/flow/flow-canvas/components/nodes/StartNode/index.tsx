...
import NodeResizeControl from '@/pages/flow/flow-canvas/components/node-resize-control';
          ...
          <Flex justify="space-between" align="center" className={styles.variable} key={item.name}>
            <Space size={3}>
              ...
              {item.rules && item.rules.length > 0 && (
                <label style={{ marginLeft: '5px', color: token.colorPrimary }}>({item.rules.length} rules)</label>
              )}
            </Space>
