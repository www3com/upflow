...
export default memo((node: NodeType<CaseNodeType>) => {
    ...
    const elseRefCallback = useCallback((el: HTMLSpanElement | null) => {
...
export const CaseSection = memo(({count, index, item, keywordRef}: {
    ...
    const optimizedKeywordRef = useCallback((el: HTMLSpanElement | null) => {
        if (keywordRef) {
            keywordRef(el);
        }
    }, [keywordRef]);
    ...
    const keywordElement = useMemo(() => {
        ...
        return <span className={styles.keyword} ref={optimizedKeywordRef}>{keyword}</span>;
    ...
    const conditionElements = useMemo(() => {
        return item.conditions.map((condition, condIndex) => {
            ...
            return (
                <Flex key={`${condition.varId}-${condition.opr}-${condition.value}-${condIndex}`}
                      align="center"
                      gap={15}
