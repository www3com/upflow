...
export const useFlow = () => {
  ...
  const onNodesChange = useCallback(
    (changes: NodeChange[]) => {
      // 防护措施：如果当前没有节点数据，且变化会导致清空，则忽略此次变化
      const currentNodes = snap.nodes as Node[];
      ...
      if (removeChanges.length > 0 && snap.selectedNode) {
        // 检查当前选中的节点是否被删除
        const isSelectedNodeRemoved = removeChanges.some((change) => change.id === snap.selectedNode?.id);
    ...
    [snap.nodes, snap.selectedNode],
  );

  const onEdgesChange = useCallback(
    (changes: EdgeChange[]) => {
      setEdges(applyEdgeChanges(changes, snap.edges as Edge[]) as EdgeType<any>[]);
    },
    [snap.edges],
  );
  ...
  const onConnect = useCallback(
    (connection: Connection) => {
      setEdges(addEdge(connection, snap.edges as EdgeType<any>[]));
    },
    [snap.edges],
  );
