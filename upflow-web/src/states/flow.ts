...
export const importDSL = (file: File) => {
  ...
  reader.onload = (e) => {
    try {
      ...
      // 验证节点数据结构
      const invalidNodes = flowData.nodes.filter((node: any) => !node.id || !node.type || !node.position || !node.data);
      ...
      // 验证边数据结构
      const invalidEdges = flowData.edges.filter((edge: any) => !edge.id || !edge.source || !edge.target);
